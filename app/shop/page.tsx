...
export default async function Shop({ searchParams }: { searchParams: Promise<{ category?: string; q?: string }> }) {
    ...
    const filtered = products.filter((product) => (!params.category || product.category === params.category) && (!params.q || product.name.toLowerCase().includes(params.q.toLowerCase())));
