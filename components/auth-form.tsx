...
export function AuthForm({ mode }: { mode: Mode }) {
    ...
    async function submit(formData: FormData) {
        ...
        const result = mode === "login" ? await supabase.auth.signInWithPassword({ email, password }) : mode === "register" ? await supabase.auth.signUp({ email, password, options: { data: { full_name: String(formData.get("name") ?? "") } } }) : await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/account` });
        setLoading(false);
        if (result.error) { setError(result.error.message); return; }
        if (mode === "forgot") setMessage("Check your email for a secure reset link."); else router.push("/account");
    }
