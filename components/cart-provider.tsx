...
type CartContextValue = { items: CartItem[]; count: number; add: (product: Product) => void; remove: (id: string) => void; update: (id: string, quantity: number) => void };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    return <CartContext.Provider value={{ items, count: items.reduce((sum, item) => sum + item.quantity, 0), add, remove, update }}>{children}</CartContext.Provider>;
}
