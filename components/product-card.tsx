...
export function ProductCard({ product }: { product: Product }) {
    ...
    return <article className="product-card">
        <div className="product-image-wrap"><Link href={`/product/${product.slug}`}><Image src={product.image} alt={product.name} fill sizes="(max-width: 700px) 50vw, 25vw" className="product-image" /></Link>{product.badge && <span className="product-badge">{product.badge}</span>}{(!checked || !isAdmin) && <button className="wishlist-btn" aria-label={`Save ${product.name}`}><Heart size={17} /></button>}{checked && !isAdmin && <button className="quick-add" onClick={() => add(product)} aria-label={`Add ${product.name} to cart`}><Plus size={18} /></button>}</div>
