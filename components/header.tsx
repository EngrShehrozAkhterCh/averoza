...
export function Header() {
    ...
    const { count } = useCart();
    useEffect(() => { const supabase = getSupabaseBrowser(); if (!supabase) return; supabase.auth.getUser().then(({ data }) => { setSignedIn(Boolean(data.user)); setEmail(data.user?.email ?? ""); }); const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => { setSignedIn(Boolean(session?.user)); setEmail(session?.user?.email ?? ""); }); return () => listener.subscription.unsubscribe(); }, []);
